...
public final class LCTime implements java.io.Serializable,Comparable{
	static final long serialVersionUID = 7886265777567493523L;
public LCTime() {
	super();
}
public LCTime(long m) {
 ...
 */
public LCTime(String strTime) {
	this(strTime,true);
}
public LCTime(java.sql.Date date) {
	this( (java.util.Date)date );
}
public LCTime(java.util.Date date) {
...
public boolean after(LCTime when) {
	return value.compareTo(when.toString()) > 0;
}

public boolean before(LCTime when) {
	return value.compareTo(when.toString()) < 0;
}

public Object clone() {
	return new LCTime(value);
}

public int compareTo(LCTime when) {
	return value.compareTo(when.toString());
}
public boolean equals(Object o) {
	if ((o != null) && (o instanceof LCTime)) {
...
public LCTime(String strTime, boolean isParse) {
	if (isParse)
